...
def load(name):
    try:
        ...
        # Numeric conversion for financial columns
        num_cols = ['Debit', 'Credit', 'Amount', 'Freight', 'HiredCharges', 'Diesel', 'Toll', 'DriverExp', 'Other', 'Profit']
        for col in num_cols:
...
if menu == "0. Dashboard":
    st.markdown("<h2 style='text-align: center; color: #00d4ff;'>📊 VIRAT LOGISTICS DASHBOARD</h2>", unsafe_allow_html=True)
    ...
    df_tf = df_t.copy(); df_pf = df_p.copy(); df_oef = df_oe.copy()
    if not df_tf.empty: df_tf['FY'] = df_tf['Date'].apply(get_fy); df_tf = df_tf[df_tf['FY'] == selected_fy]
    if not df_pf.empty: df_pf['FY'] = df_pf['Date'].apply(get_fy); df_pf = df_pf[df_pf['FY'] == selected_fy]
    if not df_oef.empty: df_oef['FY'] = df_oef['Date'].apply(get_fy); df_oef = df_oef[df_oef['FY'] == selected_fy]

    # --- Calculations ---
    total_opening_cash = 0; op_party_rec = 0; op_broker_pay = 0
    if not df_p.empty:
        op_entries = df_p[df_p['Type'] == 'OP_BAL']
        # Cash/Bank Balance
        cb_op = op_entries[op_entries['Account_Name'].str.contains('BANK|CASH', case=False, na=False)]
        total_opening_cash = cb_op['Debit'].sum() - cb_op['Credit'].sum()
        # Party/Broker Opening
        p_list = gl("Party"); b_list = gl("Broker")
        op_party_rec = op_entries[op_entries['Account_Name'].isin(p_list)]['Debit'].sum()
        op_broker_pay = op_entries[op_entries['Account_Name'].isin(b_list)]['Credit'].sum()

    # Actual Cash Flow
    cash_in = df_pf[df_pf['Type'] != 'OP_BAL']['Credit'].sum() if not df_pf.empty else 0
    cash_out = df_pf[df_pf['Type'] != 'OP_BAL']['Debit'].sum() if not df_pf.empty else 0
    oe_total = df_oef['Amount'].sum() if not df_oef.empty else 0
    # Note: Trip expenses are already in cash_out as they are auto-saved to payments.
    
    current_cash_in_hand = (total_opening_cash + cash_in) - (cash_out + oe_total)

    # Accrual Profit
    total_freight = df_tf['Freight'].sum() if not df_tf.empty else 0
    total_hired = df_tf['HiredCharges'].sum() if not df_tf.empty else 0
    trip_exp_accrual = df_tf[['Diesel', 'Toll', 'DriverExp']].sum().sum() if not df_tf.empty else 0
    net_accrual_profit = total_freight - total_hired - trip_exp_accrual - oe_total

    # Payables & Receivables Logic
    party_rec = (op_party_rec + total_freight) - df_pf[df_pf['Account_Name'].isin(gl("Party"))]['Credit'].sum()
    broker_pay = (op_broker_pay + total_hired) - df_pf[df_pf['Account_Name'].isin(gl("Broker"))]['Debit'].sum()

    # Metrics Display
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Net Cash in Hand", f"₹{current_cash_in_hand:,.0f}", delta="Actual Liquidity")
    m2.metric("Total Receivables", f"₹{party_rec:,.0f}", help="From Parties")
    m3.metric("Total Payables", f"₹{broker_pay:,.0f}", help="To Brokers/Trucks", delta_color="inverse")
    m4.metric("Accrual Profit", f"₹{net_accrual_profit:,.0f}", delta="F.Y. Performance")

    st.divider()
    # Bank wise balances
    st.subheader("🏦 Bank wise Balance")
    banks = gl("Bank")
    if banks:
        b_cols = st.columns(len(banks))
        for i, b in enumerate(banks):
            b_op = df_p[(df_p['Account_Name'] == b) & (df_p['Type'] == 'OP_BAL')]['Debit'].sum() - df_p[(df_p['Account_Name'] == b) & (df_p['Type'] == 'OP_BAL')]['Credit'].sum()
            b_in = df_pf[(df_pf['Bank_Used'] == b) | (df_pf['Account_Name'] == b)]['Credit'].sum()
            b_out = df_pf[(df_pf['Bank_Used'] == b) | (df_pf['Account_Name'] == b)]['Debit'].sum()
            b_oe = df_oef[df_oef['Bank_Used'] == b]['Amount'].sum()
            curr_b = (b_op + b_in) - (b_out + b_oe)
            b_cols[i].metric(b, f"₹{curr_b:,.0f}")

    st.divider()
    # Vehicle wise profit (Own Fleet)
    st.subheader("🚛 Vehicle-wise Profit (Own Fleet Only)")
    df_own = df_tf[df_tf['Type'].str.contains('Own', case=False, na=False)]
    if not df_own.empty:
        v_profit = df_own.groupby('Vehicle')['Profit'].sum().reset_index()
        fig_v = px.bar(v_profit, x='Vehicle', y='Profit', text_auto='.2s', color='Profit', color_continuous_scale='RdYlGn')
        st.plotly_chart(fig_v, use_container_width=True)
    else:
        st.info("No own fleet trips found for this year.")
if menu == "1. Masters Setup":
    ...
    st.header("⚖️ Accounting & Statements")
    t1, t2 = st.tabs(["💸 Add Transaction", "📖 Full Statement"])
    
    with t1:
        with st.form("p_form", clear_on_submit=True):
            f1, f2, f3 = st.columns(3)
            p_d = f1.date_input("Date")
            acc = f1.selectbox("Account*", ["Select"] + gl("Party") + gl("Broker") + gl("Bank"))
            p_t = f2.selectbox("Type", ["Receipt", "Payment", "Opening Balance"])
            p_dr = f2.number_input("Debit (Advance/Gaya)", min_value=0.0)
            p_cr = f3.number_input("Credit (Recd/Aaya)", min_value=0.0)
            p_m = f3.selectbox("Mode", ["Cash", "NEFT", "UPI", "Cheque"])
            p_bank = f3.selectbox("Bank to Update", ["N/A"] + gl("Bank"))
            
            if st.form_submit_button("Save Transaction"):
                if acc != "Select":
                    typ = "OP_BAL" if p_t == "Opening Balance" else p_t
                    if save("payments", [str(p_d), acc, typ, p_dr, p_cr, p_m, "Manual Entry", "", p_bank]):
                        st.cache_data.clear(); st.success("Saved!"); st.rerun()

    with t2:
        sel_a = st.selectbox("Select Account", ["Select"] + gl("Party") + gl("Broker") + gl("Bank"))
        if sel_a != "Select":
            df_p = load("payments"); df_t = load("trips")
            ledger = []
            # 1. Opening
            op = df_p[(df_p['Account_Name'] == sel_a) & (df_p['Type'] == 'OP_BAL')]
            for _, r in op.iterrows(): ledger.append({'Date': r['Date'], 'Particulars': 'Opening Balance', 'Debit': r['Debit'], 'Credit': r['Credit']})
            # 2. Trips
            if sel_a in gl("Party"):
                tps = df_t[df_t['Party'] == sel_a]
                for _, r in tps.iterrows(): ledger.append({'Date': r['Date'], 'Particulars': f"LR {r['LR No']}", 'Debit': r['Freight'], 'Credit': 0})
            if sel_a in gl("Broker"):
                tps = df_t[df_t['Broker'] == sel_a]
                for _, r in tps.iterrows(): ledger.append({'Date': r['Date'], 'Particulars': f"LR {r['LR No']}", 'Debit': 0, 'Credit': r['HiredCharges']})
            # 3. Payments
            pts = df_p[(df_p['Account_Name'] == sel_a) & (df_p['Type'] != 'OP_BAL')]
            for _, r in pts.iterrows(): ledger.append({'Date': r['Date'], 'Particulars': f"{r['Type']} via {r['Mode']}", 'Debit': r['Debit'], 'Credit': r['Credit']})
            
            # 4. Bank specific flow from expenses
            if sel_a in gl("Bank"):
                df_oe = load("office_expenses")
                if not df_oe.empty:
                    oe_b = df_oe[df_oe['Bank_Used'] == sel_a]
                    for _, r in oe_b.iterrows(): ledger.append({'Date': r['Date'], 'Particulars': f"Exp: {r['Category']}", 'Debit': r['Amount'], 'Credit': 0})

            if ledger:
                ldf = pd.DataFrame(ledger).sort_values('Date')
                # Running Balance
                is_broker = sel_a in gl("Broker")
                if is_broker: ldf['Balance'] = (ldf['Credit'] - ldf['Debit']).cumsum()
                else: ldf['Balance'] = (ldf['Debit'] - ldf['Credit']).cumsum()
                st.dataframe(ldf, use_container_width=True)
                st.metric("Net Balance", f"₹{ldf['Balance'].iloc[-1]:,.0f}")
