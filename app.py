...
# --- 1. CONNECTION & LOAD ---
st.set_page_config(page_title="Virat Logistics ERP v3.4", layout="wide")
...
if menu == "1. Masters Setup":
    ...
    with st.form("master_form_v34", clear_on_submit=True):
        c1, c2 = st.columns(2)
        with c1:
            name = st.text_input(f"{m_type} Name*"); gst = st.text_input("GST No / Branch Code")
        ...
        if st.form_submit_button("Add Master"):
            if name: save_row("masters", [m_type, name, gst, address, contact, ac_no, ifsc, drv_name, drv_no]); st.success("Saved!"); st.rerun()
    st.dataframe(df_m[df_m['Type'] == m_type], use_container_width=True)

elif menu == "2. LR Entry":
    st.header("📝 Professional LR Entry v3.4")
    ...
    branches = get_list('Branch'); banks = get_list('Bank'); parties = get_list('Party')
    vehicles = get_list('Vehicle'); brokers = get_list('Broker')

    # --- TOP SELECTION ---
    st.markdown("### 🏢 Business Unit & Logic")
    ...
    with col_u1:
        ...
        br_code = br_info.get('GST', 'VL') # Using GST field as branch code if needed
    with col_u2:
    ...
    with col_u3:
        # AUTO NUMBERING WITH BRANCH CODE
        auto_no = f"{br_code}-{date.today().strftime('%y%m%d%H%M')}" if sel_br != "Select" else ""
    ...
    with st.form(f"lr_form_final_{k}"):
        st.markdown("---")
            ...
            if sel_br != "Select" and bill_pty != "Select" and fr > 0:
                ...
                if save_row("trips", row):
                    ...
                    st.session_state.pdf_ready = {
                        ...
                        "Bank": f"{bk_d.get('Name','')} A/C:{bk_d.get('A_C_No','')} IFSC:{bk_d.get('IFSC','')}",
                        "PaidBy": paid_by, "InvNo": inv_no, "ShipTo": ship_to
                    }
