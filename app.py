...
if menu == "1. Masters Setup":
    ...
    with st.form("m_form", clear_on_submit=True):
        ...
        submit_label = "Update Master" if st.session_state.m_edit_idx is not None else "Add Master"
    ...
    with st.form(f"lr_form_{k}"):
        ...
        nw = c3.number_input("Net Wt", value=safe_float(ed.get('Weight', 0.0)))
        fr = c3.number_input("Freight", value=safe_float(ed.get('Freight', 0.0)))
        show_fr = c3.checkbox("Show Freight in PDF?", value=True)
        if v_cat == "Own Fleet": 
            dsl, toll, drv, hc = c1.number_input("Diesel"), c2.number_input("Toll"), c3.number_input("Adv"), 0.0
        else: 
