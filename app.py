...
# --- 1. SETUP ---
st.set_page_config(page_title="Virat ERP v8.1", layout="wide")
...
def generate_pro_pdf(lr):
    ...
    # Header Section
    elements.append(Paragraph(f"<font size=20><b>{lr.get('BrName', 'VIRAT LOGISTICS')}</b></font>", styles['Title']))
    elements.append(Paragraph(f"GST No: {lr.get('BrGST', '')} | Branch: {lr.get('BrCode', '')}", styles['Normal']))
    elements.append(Paragraph(f"Address: {lr.get('BrAddr', '')}", styles['Normal']))
    ...
    party_data = [
        ...
        [Paragraph(lr.get('Consignor', ''), styles['Normal']), 
         Paragraph(lr.get('Consignee', ''), styles['Normal']), 
         Paragraph(lr.get('Party', ''), styles['Normal'])]
    ]
    ...
    mat_data = [
        ...
        ["1", lr.get('Material', ''), lr.get('Articles', '0'), lr.get('Weight', '0'), f"Rs. {lr.get('Freight', '0')}"]
    ]
    ...
    elements.append(Paragraph(f"<b>Route:</b> {lr.get('From', '')} to {lr.get('To', '')} | <b>Bank:</b> {lr.get('Bank', '')}", styles['Normal']))
...
if menu == "Master Settings":
    ...
    with st.form("m_v81"):
        c1, c2 = st.columns(2)
        n = c1.text_input("Name*"); g = c1.text_input("GST Number")
        code = c2.text_input("Branch Code (BC-01)"); addr = c2.text_area("Address")
        if st.form_submit_button("Save Master"):
            if n: sh.worksheet("masters").append_row([m_type, n, g, addr, code, "", "", "", ""]); st.rerun()
    st.divider()
    if not df_m.empty:
        for i, r in df_m[df_m['Type'] == m_type].iterrows():
            mc1, mc2 = st.columns([5, 1])
            mc1.write(f"**{r['Name']}** | {r.get('Code', r.get('Branch Code', ''))}")
            if mc2.button("🗑️ Delete", key=f"md_{i}"):
    ...
    v_cat = c2.radio("Trip Type*", ["Own Fleet", "Market Hired"], horizontal=True, key=f"vcat_{k}")
    ...
    with st.form(f"f_{k}"):
        ...
        wt, f_a = f3.number_input("Weight"), f3.number_input("Freight Amount*", min_value=0.0)
    ...
    if not df_t.empty:
        ...
        for i, r in df_f.iterrows():
            with st.expander(f"LR: {r.get('LR No', '')} | {r.get('Consignee', '')}"):
                c1, c2 = st.columns(2)
                # Building dictionary for PDF
                pdf_dict = r.to_dict()
                pdf_dict['BrName'] = "VIRAT LOGISTICS" # Example values
