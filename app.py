...
if menu == "0. Dashboard":
    ...
    # Cash in Hand Minus Office Expenses
    ...
    m2.metric("Total Receivables", f"₹{receivables:,.0f}", help="Market Pending")
    m3.metric("Yearly Revenue", f"₹{total_rev:,.0f}", delta="Billed")
    ...
    with col_a:
        ...
        cf_data = pd.DataFrame({
            'Category': ['Opening Bank', 'Receipts', 'Payments', 'Trip Cash', 'Office Exp'], 
    ...
    if not df_tf.empty or not df_oef.empty:
        ...
        # Outer Join: Taaki sirf kharche wali gadiyan (0891) bhi dikhein
