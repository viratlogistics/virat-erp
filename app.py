...
def gen_pdf(name, trips, pmts, bal, lbl):
    ...
    for _, r in trips.iterrows():
        a = r.get('Freight',0) if lbl=="Party" else r.get('HiredCharges',0)
        pdf.cell(30,8,str(r.get('Date','')),1); pdf.cell(40,8,str(r.get('LR','')),1); pdf.cell(60,8,str(r.get('Vehicle','')),1)
        pdf.cell(30,8,str(a) if lbl=="Party" else "0",1); pdf.cell(30,8,"0" if lbl=="Party" else str(a),1,1)
    ...
    with st.sidebar:
        ...
        if st.button("Access ERP"):
            if u == "admin" and p == "1234":
                ...
                st.rerun()
    st.warning("Please Login to Continue"); st.stop()

menu = st.sidebar.selectbox("🚀 MENU", ["Dashboard","Add LR","LR Manager","Monthly Bill","P&L Report","Vehicle Profit","Driver Salary","Party Ledger","Broker Ledger","Record Entry"])

# --- 5. FEATURES ---
if menu == "Dashboard":
    p_in = df_p[df_p["Category"]=="Party"]["Amount"].sum()
    b_out = df_p[df_p["Category"]=="Broker"]["Amount"].sum()
    st.title("📊 Cash & Fund Dashboard")
    c1,c2,c3 = st.columns(3)
    c1.metric("Cash Collected",f"₹{p_in:,.0f}")
    c2.metric("Cash Out",f"₹{(b_out+df_a['Amount'].sum()):,.0f}")
    c3.metric("Net Cashflow",f"₹{(p_in-b_out-df_a['Amount'].sum()):,.0f}")
    st.divider()
    f1,f2 = st.columns(2)
    f1.metric("Lena Hai (Receivables)",f"₹{(df_t['Freight'].sum()-p_in):,.0f}")
    f2.metric("Dena Hai (Payables)",f"₹{(df_t['HiredCharges'].sum()-b_out):,.0f}")

elif menu == "Add LR":
    st.header("📝 Consignment Entry")
    v_t = st.radio("Select Category", ["Own Fleet", "Market Hired"], horizontal=True)
    with st.form("lr_form", clear_on_submit=True):
        ...
        if st.form_submit_button("SAVE LR"):
            ...
            row = [str(d), f"LR-{len(df_t)+1001}", v_t, pty, cnor, "", "", "", "", "", "Material", 0, v_n, "Driver", br, fl, tl, fr, hc, dsl, de, tll, 0, pf]
    ...
    for i, r in f_df.iterrows():
        with st.expander(f"📄 {r['LR']} | {r['Vehicle']}"):
            with st.form(key=f"ed_{i}"):
                ...
                if st.form_submit_button("Update"):
                    ...
                    update_lr(r['LR'], upd); st.success("Updated!"); st.rerun()
            if st.button("Delete", key=f"del_{i}"):
                sh.worksheet("trips").delete_rows(sh.worksheet("trips").find(str(r['LR'])).row)
                st.rerun()
    ...
    st.columns(3)[0].metric("Revenue", f"₹{r:,.0f}"); st.columns(3)[1].metric("Costs", f"₹{(h+dsl+a):,.0f}"); st.columns(3)[2].metric("NET PROFIT", f"₹{(r-h-dsl-a):,.0f}")

elif menu == "Vehicle Profit":
    v_r = df_t.groupby("Vehicle").agg({"LR": "count", "Freight": "sum", "Profit": "sum"}).reset_index()
    st.dataframe(v_r, use_container_width=True)

elif menu == "Driver Salary":
    with st.form("d_f"):
        dn, sl, ad = st.text_input("Driver"), st.number_input("Salary"), st.number_input("Advance")
        if st.form_submit_button("Record"): save("drivers", [str(date.today()), dn, "Present", ad, sl]); st.rerun()
    if not df_d.empty:
        ds = df_d.groupby("Name").agg({"Advance": "sum", "Salary": "max", "Date": "count"}).reset_index()
        ds["Balance"] = ((ds["Salary"]/30)*ds["Date"]) - ds["Advance"]
        st.dataframe(ds)

elif "Ledger" in menu:
    cat = "Party" if "Party" in menu else "Broker"
    col = "Party" if cat=="Party" else "Broker"
    sp = st.selectbox("Select", df_t[col].unique())
    p_t = df_t[df_t[col]==sp]; p_p = df_p[(df_p["Name"]==sp) & (df_p["Category"]==cat)]
    bl = p_t["Freight" if cat=="Party" else "HiredCharges"].sum() - p_p["Amount"].sum()
    ...
    with st.form("oe"):
        ...
        if st.form_submit_button("Save"): save("admin", [str(date.today()), "Other", am, rem]); st.rerun()
